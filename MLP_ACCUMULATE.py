...
for i in range(train_idx[0], train_idx[1]):
    xs[i-train_idx[0]] = x_samples[i].reshape(1, 45)
    ys[i-train_idx[0]] = y_samples[i].reshape(1, 45)
...
#for i in range(train_idx[0], test_idx[1]):
  #  xs_accu_tt[i]=xs_accu_tt[i]
    
for i in range(train_idx[0], test_idx[1]):
    xs_accu_tt[i]=xs_accu_tt[i]/np.mean(xs_accu_tt[i])
xs_accu_tt[0]=xs_tt[0]


        
for i in range(train_idx[0], train_idx[1]):
    xs_accu_tr[i-train_idx[0]]= xs_accu_tt[i]
...
history= model.fit(xs_accu_tr,ys,epochs=2, batch_size=1)
...
rewards = [sum(train_total_reward)/800, sum(val_total_reward)/100, sum(test_total_reward)/91]
...
pred_xs=(ys_tt[len(x_samples)-1])+xs_accu_tt[len(x_samples)-1].reshape(1,45)
